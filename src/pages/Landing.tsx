    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden bg-[#050505]">
        ...
        <div className="relative mx-auto max-w-5xl px-6 py-8 text-center md:py-12">
          <motion.div initial={heroVisible ? false : { opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1, ease: [0.16, 1, 0.3, 1] }} className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1 text-[10px] font-light tracking-widest text-white/35 uppercase backdrop-blur-sm sm:text-xs">
            Private Family Gallery
          </motion.div>
          <motion.h1 initial={heroVisible ? false : { opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2, ease: [0.16, 1, 0.3, 1] }} className="text-3xl font-semibold leading-tight text-white md:text-5xl" style={{ fontFamily: "var(--font-serif)" }}>
            Sweet Family&apos;s
            <span className="block mt-1" style={{ background: "linear-gradient(135deg, #bf953f 0%, #fcf6ba 30%, #b38728 50%, #fbf5b7 70%, #aa771c 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Photos</span>
          </motion.h1>
          <div className={`hero-divider my-5 ${dividerExpanded ? "expanded" : ""}`} />
          <motion.p initial={heroVisible ? false : { opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.5, ease: [0.16, 1, 0.3, 1] }} className="mx-auto max-w-lg text-xs leading-relaxed text-white/40 font-light sm:text-sm">
            Browse, download, and share your family&apos;s beautiful moments — in full quality, without compression or clutter.
          </motion.p>
          <motion.div initial={heroVisible ? false : { opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.65, ease: [0.16, 1, 0.3, 1] }} className="mt-4 flex flex-col items-center justify-center gap-2 sm:mt-5 sm:flex-row sm:gap-2.5">
            <a href="#gallery" className="group inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-6 py-2.5 text-xs font-medium text-white backdrop-blur-sm transition-all hover:bg-white/10 hover:border-white/20 active:scale-[0.97] sm:px-7 sm:py-3 sm:text-sm">
              <Camera className="h-4 w-4 transition-transform group-hover:scale-110" /> View Gallery
            </a>
            <button onClick={() => setShowUpload(true)} className="group inline-flex items-center gap-2 rounded-xl bg-white/10 px-6 py-2.5 text-xs font-medium text-white backdrop-blur-sm transition-all hover:bg-white/15 active:scale-[0.97] sm:px-7 sm:py-3 sm:text-sm">
              <Upload className="h-4 w-4 transition-transform group-hover:scale-110 group-hover:-translate-y-0.5" /> Upload Photos
            </button>
          </motion.div>
          <motion.div initial={heroVisible ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 1.2 }} className="mt-4 sm:mt-6">
      ...
      <section id="gallery" className="mx-auto max-w-7xl px-6 pt-4 pb-16 md:pt-6 md:pb-24">
        <div className="mb-8 text-center md:mb-10">
          ...
          {photos.length > 0 && (
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.6 }} className="mt-2 text-xs text-muted-foreground sm:text-sm">
